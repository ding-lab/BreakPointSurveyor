...
NOTE: `./data` is not a good option if ./data already exists.

```
docker pull mwyczalkowski/breakpoint_surveyor
mkdir ./data_docker
docker run -v $PWD/data_docker:/data -it mwyczalkowski/breakpoint_surveyor
```

This will download the container and then run it.  Data directory will be in `./data`. Once the container is running,
run the entire Synthetic workflow with,

```
cd /usr/local/BreakPointSurveyor
git checkout Synthetic  # this ends up being weird
./run_BPS A* ... etc
```
NOTE: git checkout Synthetic does not work as expected, leaves `master` directories hanging around
