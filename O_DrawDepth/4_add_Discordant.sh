...
function process_chrom {
    ...
    #BPC="$DATD/BPC/${BAR}.Discordant.BPC.chromB.dat"
    BPC="$OUTD/ICB/${NAME}.${CHROM_ID}.ICB.dat"
...
#    ARGS="$ARGS -N ${N_CHROM}:${N_START}-${N_END}"
    ...
    # To use attribute to define color, simply don't define the color
    #COLOR="-c #377EB8 -z 2.5 -a 0.05 -s 16"
    COLOR="-z 2.5 -a 0.05 -s 16"
